...
while getopts ':hi:b:m:o:c:l:sr:d:p:a:t:' opt
do
    case ${opt} in
	h)
	    echo -e 'Options:'
	    echo -e '\t -i Path To Raw Fastq Directory [Mandatory]'
	    echo -e '\t -b Barcodes File (eg. QiimePrepTemplate) [Mandatory]'
		...
		echo -e '\t -c Specify Name Of Column In Metadata Containing Golay Barcode Sequences For Demultiplexing [Default: BARCODE]'
	...
	s) s_flag='true' ;;
	t) t_flag=$OPTARG ;;
	\?) echo -e '\t Usage: 16S_pipeline -i PathToRawFastqDirectory -b BarcodesFile -m MetadataFile\n\tOR\n\tHelp and Optional Arguments: 16S_pipeline.sh -h\n' >&2
	    exit 1
	    ;;
...
if [ "$i_flag" = '' ] || [ "$b_flag" = '' ] || [ "$m_flag" = '' ]
then
    echo 'ERROR'
    echo 'Usage: 16S_pipeline -i PathToRawFastqDirectory -b BarcodesFile -m MetadataFile'
    echo 'Use -h for full options list'
    echo ''
    exit 1
else
    echo 'Path To Raw Fastq Directory: '$i_flag
    echo 'Barcodes File: '$b_flag
    echo 'Metadata File: '$m_flag
	echo 'Output Directory: '$o_flag
	echo 'Threads or Parallel Jobs To Use: '$t_flag
fi

csvtk headers -t $b_flag | grep -x -q $c_flag && var_temp='yes' || var_temp='no'
if  [[ $var_temp = 'yes' ]]
then
    echo 'Barcodes Column: '$c_flag
else
    echo $c_flag' Column Not Present in Metadata'
	inputerror='true'
fi
...
qiime tools import --type EMPPairedEndSequences --input-path $i_flag --output-path raw-seqs.qza

qiime demux emp-single \
	--i-seqs raw-seqs.qza \
	--m-barcodes-file $b_flag \
	--m-barcodes-column $c_flag \
	--p-rev-comp-barcodes \
	--p-rev-comp-mapping-barcodes \
	--o-per-sample-sequences demux.qza \
	--o-error-correction-details demux-errorcorrectiondetails.qza
...
then
	...
	then
		...
		mv demux-errorcorrectiondetails.qza demux-filtered.qza demux-filterstats.qza demux-filterstats.qzv demux.qza demux.qzv $o_flag
...
then
	mv deblur.log deblur-stats.qza $o_flag
	mv demux-errorcorrectiondetails.qza demux-filtered.qza demux-filterstats.qza demux-filterstats.qzv demux.qza demux.qzv $o_flag
	mv feature-table.qza feature-table.qzv feature-table-predecontam.qza contaminant_otus.csv rep-seqs.qza $o_flag
